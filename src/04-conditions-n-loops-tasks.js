...
function getDigitalRoot(num) {
  const result = (e) => {
    let sum = Array.from(String(e), Number);
    sum = sum.reduce((acc, value) => acc + value);
    return String(sum).length === 1 ? sum : result(sum);
  };
  return result(num);
  // throw new Error('Not implemented');
}
...
function isBracketsBalanced(str) {
  ...
  const openBr = ['[', '(', '{', '<'];
  const closeBr = [']', ')', '}', '>'];
  const bracketsArr = Array.from(str);

  for (let i = 0; i < bracketsArr.length; i += 1) {
    const currentBr = bracketsArr[i];
